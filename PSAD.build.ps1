param (
    ...
    [string]$BuildFile = (Join-Path $BuildRoot 'build\PSAD.buildenvironment.ps1'),
...
task AutoIncreaseVersionBuildLevel -after PublishPSGallery -if {$Script:BuildEnv.OptionUpdateVersionAfterPublishing} {
    ...
    if (($AllReleases -contains $ProposedNewRelease) -and (-not $Force) -and (-not $Script:BuildEnv.Force)) {
        Write-Build Red "The module version already has been released (the folder exists within the releases folder. In order to set your build project to $($ProposedNewRelease.ToString()) you will need to pass the -Force switch to this build script with the -NewVersion parameter" -Level 2
    ...
    Write-Description White 'Updating module manifest with new module version' -level 2
    try {
        Update-ModuleManifest -Path $ModuleManifestFullPath -ModuleVersion $ProposedNewRelease
    }
    catch {
        throw $_
    }
...
task InstallModule VersionCheck, {
    ...
    if (Test-Path $ModuleInstallPath) {
        ...
        if ($Force -or $Script:BuildEnv.Force) {
