...
if ((Get-Variable 'BuildEnv' -ErrorAction:SilentlyContinue) -eq $null) {
    $Script:BuildEnv = New-Object -TypeName PSObject -Property @{
        ...
        ModuleToBuild = 'PSAD'
        ModuleVersion = '0.0.1'
        ModuleWebsite = 'https://github.com/zloeber/PSAD'
        ModuleCopyright = "(c) $((get-date).Year.ToString()) Zachary Loeber. All rights reserved."
        ModuleLicenseURI = 'https://github.com/zloeber/PSAD/LICENSE.md'
        ModuleTags = 'ADSI,Active Directory' -split ','
        ModuleAuthor = 'Zachary Loeber'
        ModuleDescription = 'Advanced ADSI PowerShell Module'
        ...
        # You should not actually enter this key here but should manually enter it in the PSAD.buildenvironment.json file
    ...
    ########################################
    $PersistentBuildFile = join-path $PSScriptRoot "PSAD.buildenvironment.json"
